...
int main() {
    ...
    while(1) {
        ...
        for(int i = 0; i < num_Nodes; i++) {
            string temp;
            getline(cin, temp);
            vector<string> temp_Split = parse(temp);
            for(int j = 0; j < temp_Split.size(); j++) { //for every number in the row (in_edges), increase the amount of inedges for the corressponding node
                stringstream n_N(temp_Split[j]);
                int n_Node = 0;
                n_N >> n_Node;
                n[n_Node]++;
            }
